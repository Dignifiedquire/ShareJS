...
class Connection extends EventEmitter {
  constructor(socket) {
    ...
    // TODO refactor queries access to Map interface!!
  ...
  handleMessage(msg) {
    ...
    case 'qunsub':
      ...
      if (this.queries.has(id)) {
        this.queries(id)._onMessage(msg)
      }
      break
  ...
  _setupRetry() {
    ...
    this._retryInterval = setInterval(() => {
      this.collections.forEach((collection, cName) => {
        collection.forEach(doc => doc.retry())
      })
    }, 1000)
  }
  ...
  bsEnd() {
    ...
    if (Object.keys(this.subscribeData).length > 0) {
  ...
  get(collection, name, data) {
    ...
    const collectionObject = this.collections.get(collection)

    if (!collectionObject.has(name)) {
      collectionObject.set(name, new Doc(this, collection, name))
    }

    const doc = collectionObject.get(name)
   ...
   * @private
   */
  _destroyDoc({collection, name}) {

    if (!this.collection.has(collection)) return

    const collectionObject = this.collection.get(collection)

    collectionObject.delete(name)
