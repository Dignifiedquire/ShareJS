...
// Try to compose data2 into data1. Returns truthy if it succeeds,
// otherwise falsy.
export const tryCompose = (type, data1, data2) => {
  if (data1.create && data2.del) {
    ...
    let data
    if (data1.create.data === undefined) {
      data = type.create()
    } else {
      data = data1.create.data
    }
...
// Transform server op data by a client op, and vice versa.
// Ops are edited in place.
export const xf = (client, server) => {
  ...
  if (client.create) {
    throw new Error(
      `Invalid state. This is a bug. ${this.collection} ${this.name}`
    )
  }
  ...
  if (client.type.transformX) {
    const result = client.type.transformX(client.op, server.op)
    client.op = result[0]
    server.op = result[1]
  } else {
    const _c = client.type.transform(client.op, server.op, 'left')
    const _s = client.type.transform(server.op, client.op, 'right')
    client.op = _c
    server.op = _s
  }
}
