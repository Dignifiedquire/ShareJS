...
class Doc extends EventEmitter {
  constructor(connection, collection, name) {
    ...
    // The data the document object stores can be in one of the following
    // three states:
    //   - No data. (null) We honestly don't know whats going on.
    //   - Floating ('floating'): we have a locally created document that
    //     hasn't been created on the server yet)
    //   - Live ('ready') (we have data thats current on the server at
    //     some version).
    this.state = null

    // Our subscription status. Either we're subscribed on the server, or
    // we aren't.
    ...
    // The editing contexts. These are usually instances of the type API
    // when the document is ready for edits.
    ...
    // This used to just be a single operation, but creates & deletes can't
    // be composed with regular operations.
    //
  ...
  // this function exists to update the document based on any added &
  // removed API methods.
  //
  ...
  _setType(newType) {
    if (typeof newType === 'string') {
      if (!types[newType]) {
        throw new Error(
          `Missing type ${newType} ${this.collection} ${this.name}`
        )
      }
  ...
  // This function exists so connection can call it directly for
  // bulk subscribes. It could just make a temporary object literal,
  // thats pretty slow.
  _handleSubscribe(err, data) {
  ...
  // This is called by the connection when it receives a message
  // for the document.
  _onMessage(msg) {
    if (msg.c !== this.collection || msg.d !== this.name) {
      ...
      throw new Error(
        `Got message for wrong document. ${this.collection} ${this.name}`
      )
    }
    ...
    case 'ack':
      ...
      if (msg.error && msg.error !== 'Op already submitted') {
        // The server has rejected an op from the client for
        // an unexpected reason.
        // We'll send the error message to the user and try to roll
        // back the change.
        if (this.inflightData) {
          console.warn(
            `Operation was rejected (${msg.error}).
             Trying to rollback change locally.`
          )
          this._tryRollback(this.inflightData)
          this._clearInflightOp(msg.error)
        } else {
          // I managed to get into this state once. I'm not
          // sure how it happened. The op was maybe double-acknowledged?
  ...
  // These functions iare copied into the query class as well, so
  // be careful making changes here.

  // Value is true, false or 'fetch'.
  _setWantSubscribe(value, callback, err) {
  ...
  _finishSub(err) {
    ...
    for (let i = 0; i < this._subscribeCallbacks.length; i++) {
  ...
  // If the type supports incremental updates and `this.incremental` is true
  // we fire `op` after every small operation.
  //
  ...
  _otApply(opData, context) {
    ...
    if (opData.create) {
      ...
      let create = opData.create
      this._setType(create.type)
      this.snapshot = this.type.create(create.data)

      // This is a bit heavyweight, but I want the created event to fire
      // outside of the lock.
      this.once('unlock', () => this.emit('create', context))
    } else if (opData.del) {
      ...
      if (!this.type) {
        throw new Error(
          `Document does not exist. ${this.collection} ${this.name}`
        )
      }
      ...
      for (let i = 0; i < this.editingContexts.length; i++) {
        let c = this.editingContexts[i]
      ...
      if (this.incremental && type.incrementalApply) {
        type.incrementalApply(this.snapshot, op, (o, snapshot) => {
          this.snapshot = snapshot
          this.emit('op', o, context)
        })
      } else {
        // This is the most common case, simply applying the operation to
        // the local snapshot.
        this.snapshot = type.apply(this.snapshot, op)
        this.emit('op', op, context)
      }
    }
    // Its possible for none of the above cases to match, in which case
    // the op is a no-op. This will happen when a document has been deleted
    // locally and remote ops edit the document.

    this.locked = false
    this.emit('unlock')

    if (opData.op) {
      let contexts = this.editingContexts
      ...
      for (let i = 0; i < contexts.length; i++) {
        if (contexts[i].shouldRemove()) contexts.splice(i--, 1)
      }
  ...
  _sendOpData() {
    let d = this.inflightData

    if (this.action) {
      throw new Error(
        `Invalid state ${this.action} for sendOpData.
  ...
  //
  // Internal method called to do the actual work for submitOp(),
  // create() and del().
  // @private
  //
  // @param opData
  ...
  _submitOpData(opData, context, callback) {
    ...
    if (this.locked) {
      return error(
        `Cannot call submitOp from inside an 'op' event handler.
        ${this.collection} ${this.name}`
      )
    }

    // The opData contains either op, create, delete, or none of
    // the above (a no-op).
    if (opData.op) {
      if (!this.type) return error('Document has not been created')
      // Try to normalize the op. This removes trailing skip:0's and
      // things like that.
    ...
    let operation
    const previous = this.pendingData[this.pendingData.length - 1]
  ...
  //
  // This does NOT get called if our op fails to reach the server for some
  // reason - we optimistically assume it'll make it there eventually.
  _tryRollback(opData) {
    // This is probably horribly broken.
    if (opData.create) {
      this._setType(null)

      // I don't think its possible to get here if we aren't in a floating
      // state.
      ...
      this.emit(
        'error',
        'Op apply failed and the operation could not be reverted'
      )
  ...
  _clearInflightOp(error) {
    const callbacks = this.inflightData.callbacks
    const context = this._opErrorContext()
    // There's no nice way to pass this context back to the caller -
    // I settled on using simple strings for error messages, and now
    // this is hurting me. I'll fix this API in sharejs 0.8.
  ...
  _opAcknowledged(msg) {
    // Our inflight op has been acknowledged, so we can throw away
    // the inflight data. (We were only holding on to it incase we needed
    // to resend the op.)
    if (!this.state) {
      throw new Error(
        `opAcknowledged called from a null state. This should
        never happen. ${this.collection} ${this.name}`
      )
    } else if (this.state === 'floating') {
      if (!this.inflightData.create) {
        throw new Error(
          `Cannot acknowledge an op. ${this.collection} ${this.name}`
        )
      }

      // Our create has been acknowledged. This is the same as
      // ingesting some data.
      this.version = msg.v
      this.state = 'ready'
      ...
      if (msg.v !== this.version) {
        throw new Error(
          `Invalid version from server. This can happen when you submit
          ops in a submitOp callback. Expected: ${this.version} Message
          version: ${msg.v} ${this.collection} ${this.name}`
        )
      }
    }
  ...
  // @param data.type
  // @fires ready
  ingestData({v, type, data}) {
    if (typeof v !== 'number') {
      throw new Error(
        `Missing version in ingested data ${this.collection} ${this.name}.`
      )
    }
    ...
    if (this.state) {
      ...
      console.warn(
        `Ignoring ingest data for ${this.collection} ${this.name}
         in state: ${this.state}
         version: ${this.version}
         snapshot: ${this.snapshot}
         incoming data: ${{v, data, type}}
        `
      )
      return
    }
  ...
  retry() {
    ...
    const threshold = 5000// Math.pow(2, this.inflightData.retries)
  ...
  hasPending() {
    const hasAction = this.action != null
    const hasInflightData = this.inflightData != null
    const hasPendingData = !!this.pendingData.length

    return hasAction || hasInflightData || hasPendingData
  }

  // Send the next pending op to the server, if we can.
  //
  // Only one operation can be in-flight at a time. If an operation
  // is already on its way, or we're not currently connected, this
  // method does nothing.
  flush() {
    ...
    while (this.pendingData.length && isNoOp(opData = this.pendingData[0])) {
      const callbacks = opData.callbacks
      for (let i = 0; i < callbacks.length; i++) {
    ...
    const hasPendingData = this.pendingData.length
    const isConnected = this.connection.state === 'connected'
    if (!this.paused && hasPendingData && isConnected) {
      ...
      this.action = 'fetch'
      let op = {a: this.action}
      if (this.state === 'ready') op.v = this.version
      this._send(op)
    } else if (!this.subscribed && this.wantSubscribe) {
      this.action = 'subscribe'
      // Special send method needed for bulk subscribes on reconnect.
      const version = this.state === 'ready' ? this.version : null
  ...
  submitOp(op, context, callback) {
    this._submitOpData({op: op}, context, callback)
  }

  // Create the document, which in ShareJS semantics means to set its
  // type. Every object implicitly exists in the database but has no data
  // and no type. Create sets the type of the object and can optionally set
  // some initial data on the object, depending on the type.
  //
  // @param type  OT type
  // @param data  initial
  // @param context  editing context
  // @param callback  called when operation submitted
  create(type, data, context, callback) {
    if (typeof data === 'function') {
      // Setting the context to be the callback function in this case so
      // _submitOpData can handle the default value thing.
      context = data
      data = undefined
    }

    const op = {create: {type: type, data: data}}
    if (this.type) {
      if (callback) {
        callback('Document already exists', this._opErrorContext(op))
      }
      return
    }
  ...
  destroy(callback) {
    this.unsubscribe(() => {
      ...
      if (this.hasPending()) {
        this.once('nothing pending', () => {
          this.connection._destroyDoc(this)
        })
      } else {
        this.connection._destroyDoc(this)
      }
