...
class Query extends EventEmitter {
  constructor(type, connection, id, collection, query, options, callback) {
    // 'fetch' or 'sub'
    this.type = type
    ...
    // just polling the changed item). This needs to be enabled to be able
    // to use ordered queries (sortby:) and paginated queries. Set to
    // undefined, it will be enabled / disabled automatically based on the
    // query's properties.
  ...
  _execute() {
    ...
    if (this.docMode) {
      collectionVersions = {}
      // Collect the version of all the documents in the current
      // result set so we don't need to be sent their snapshots again.
      for (let i = 0; i < this.knownDocs.length; i++) {
        const doc = this.knownDocs[i]
        let c = collectionVersions[doc.collection] =
  ...
  _dataToDocs(data) {
    ...
    for (let i = 0; i < data.length; i++) {
      ...
      // Types are only put in for the first result in the set and every
      // time the type changes in the list.
      if (docData.type) {
      ...
      const coll = docData.c || this.collection
      const doc = this.connection.get(coll, docData.d, docData)
  ...
  // Internal method called from connection to pass server messages to
  // the query.
  _onMessage(msg) {
    ...
    switch (msg.a) {
    case 'qfetch':
      const results = msg.data ? this._dataToDocs(msg.data) : undefined
    ...
    case 'q':
      ...
      if (msg.diff) {
        ...
        // in event callbacks.
        for (let i = 0; i < msg.diff.length; i++) {
          let d = msg.diff[i]
        ...
        for (let i = 0; i < msg.diff.length; i++) {
          ...
          switch (d.type) {
          case 'insert':
            const newDocs = d.values
            ;[].splice.apply(this.results, [d.index, 0].concat(newDocs))
            this.emit('insert', newDocs, d.index)
            break
          case 'remove':
            {
              const howMany = d.howMany || 1
              const removed = this.results.splice(d.index, howMany)
          ...
          case 'move':
            {
              const howMany = d.howMany || 1
              const docs = this.results.splice(d.from, howMany)
